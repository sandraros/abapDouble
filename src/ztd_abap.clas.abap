...
CLASS ztd_abap IMPLEMENTATION.
  ...
  METHOD get_dataset.
    rs_result = VALUE #( gt_dataset[ file_path = EXACT #( iv_file_path ) ] OPTIONAL ).
  ENDMETHOD.

  METHOD get_singleton.
    IF go_singleton IS NOT BOUND.
      go_singleton = NEW ztd_abap( ).
  ...
  METHOD zif_abap~close_dataset.
    DATA(ls_dataset) = REF #( gt_dataset[ file_path = iv_file_path ] OPTIONAL ).
    IF ls_dataset IS BOUND.
      ls_dataset->opened = abap_false.
    ENDIF.
  ENDMETHOD.

  METHOD zif_abap~delete_dataset.
    DELETE gt_dataset WHERE file_path = EXACT ts_dataset-file_path( iv_file_path ).
  ENDMETHOD.
  ...
  METHOD zif_abap~message.
    ...
    INSERT VALUE #( timestamp    = lv_timestamp
                    texte        = iv_text
                    type         = iv_mtype
                    display_like = iv_display_like )
  ...
  METHOD zif_abap~open_dataset.
    ...
    IF ls_dataset IS NOT BOUND.
      INSERT VALUE #( file_path = iv_file_path )
             INTO TABLE gt_dataset
  ...
  METHOD zif_abap~transfer.
    ...
    ENDIF.
    " Allow flat structures (so, CONV instead of EXACT).
    INSERT CONV string( iv_data ) INTO TABLE ls_dataset->content.
  ENDMETHOD.
  ...
  METHOD zif_abap~write.
    ...
    IF iv_same_line = abap_false.
      ...
      ELSE.
        INSERT repeat( val = ` `
                       occ = iv_position - 1 ) && lv_text INTO TABLE gs_internal_session->abap_list.
      ENDIF.
    ELSE.
      IF gs_internal_session->abap_list IS INITIAL.
        INSERT lv_text INTO TABLE gs_internal_session->abap_list.
      ELSE.
        DATA(lv_last_line) = REF #( gs_internal_session->abap_list[ lines( gs_internal_session->abap_list ) ] ).
        IF iv_position IS INITIAL.
